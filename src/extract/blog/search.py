...
from extract.news import dailian
...
def publish_embedded_links(main_url, platform):

    if platform == 'news':
        embedded_links = dailian.get_links(main_url)
    else:
    ...
    for item in embedded_links:
        publish(item, exchange='all_urls_exchange', routing_key='')


if __name__ == '__main__':
    # start_url = 'https://ksh-coding.tistory.com/144'
    start_url = 'https://www.dailian.co.kr'
    publish_embedded_links(start_url, 'news')
